...
-- but it may interact with other apps' keybindings.
modkey = "Mod4"
...
tags = {
  ...
  layout = { layouts[1], layouts[1], layouts[1], layouts[1],
 	     layouts[5], layouts[1], layouts[4], layouts[3], layouts[4]}
}
...
-- For the web tag, set ncol and the master window factor.
awful.tag.setncol(2, tags[1][1])
awful.tag.setmwfact(1.0, tags[1][1])

-- For each work tag, set the number of master windows to 2.
for n = 2, 5 do
    awful.tag.setnmaster(1, tags[1][n])
end

-- }}}

-- Move the mouse out of the way
mouse.coords({x=0, y=768})
...
mymainmenu = awful.menu({ items = { { "&awesome", myawesomemenu},
                                    ...
                                    { "&org", terminal .. " -e \"vim /home/thewormkill/org/notes.md\""},
                                    { "&bookmarks", terminal .. " -e \"vim /home/thewormkill/bookmarks.md\""},
                                    { "&weechat", "termite --class Chat -e weechat"},
                                    { "&mumble", "mumble" },
                                    { "&games", mygamemenu},
                                  }
                        })
...
globalkeys = awful.util.table.join(
    ...
    -- tag switching
    -- awful.key({ modkey, "Control"   }, "Left",   awful.tag.viewprev       ), -- switch tags forward
    -- awful.key({ modkey, "Control"   }, "Right",  awful.tag.viewnext       ), -- ... and backward.
    ...
    -- Layout and screen manipulation
    ...
    -- Standard program
    awful.key({ modkey,           }, "i", function () awful.util.spawn(terminal) end),
    ...
    -- Create a naughty notification with my org agenda.
    awful.key({ modkey }, "o",
   	 function()
		if org_notification == nil then
		    local agenda = awful.util.pread(
    ...
    awful.key({ modkey }, "p",
         function()
                if weather_notification == nil then
                    local weather = awful.util.pread(
		        "~/.local/bin/hweather -a $(cat ~/hweather/openweathermap.id) -f Pango -u Metric -c Aachen -C de")
                    weather_notification = naughty.notify({text = weather, timeout = 0})
                else
